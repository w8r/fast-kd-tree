...
function quickSort(array, order, left, right) {
  if (left < right) {
    var p = partition(array, order, left, right);
...
var rightColor = linear$2().domain([Math.floor(N / 2), N])
      .interpolate(interpolateRgb)
      .range(['gray', 'blue']);
...
function render() {
  ...
  while (Q.length !== 0) {
    var node$1 = Q.pop();
    if (node$1) {
      var pts = getPoints(node$1);
      var hull = polygonHull(pts.map(function (ref) {
        var x = ref.x;
        var y = ref.y;
  ...
  ctx.fillStyle = 'red';
  ctx.beginPath();
  [tree$1._root.point].forEach(function (ref) {
    var x = ref.x;
    var y = ref.y;
  ...
  var ll = getPoints(tree$1._root.left);
  var rr = getPoints(tree$1._root.left);
